...
public class AuthController {
    ...
    public String doRegister(@Valid @ModelAttribute("registerDTO") RegisterDTO dto,
        ...
        try {
            ...
            try {
                // Attempt to send email OTP for verification
                ...
                session.setAttribute("pendingRegisterOtpExpiry", System.currentTimeMillis() + (10 * 60 * 1000));
                ...
                // Email sending failed — register directly without OTP verification
                logger.warn("Email sending failed for user {}. Registering without email verification: {}",
                        dto.getUsername(), emailEx.getMessage());

                userService.register(dto);
                redirectAttrs.addFlashAttribute("warningMsg",
                        "Account created! Email verification was skipped (mail server unavailable). You can now login.");
